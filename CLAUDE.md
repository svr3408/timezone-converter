...
- **Active zones (`sourceZone`/`targetZone` vars) are the source of truth, not the input values.** Inputs can hold invalid free text. Each datalist option's value is the full label `Zone (UTC±HH:MM)`; `resolveZone` maps an input string back to the bare IANA name (accepting either the full label or a bare zone name) via the `labelToZone`/`zoneToLabel` maps. On `input`, a zone is applied only when `resolveZone` returns non-null; on `change` (blur/enter), leftover non-matching text sets `aria-invalid="true"`. `setInvalid(input, invalid)` is the single chokepoint for validity (used by both zone-input handlers and by swap): it toggles `aria-invalid` (which `style.css` keys the error border and message off — no separate CSS modifier class) and **in the same step injects/clears the error element's text**. The text must be injected on error, not merely revealed by CSS — a `role="alert"` whose static text only toggles `display` is not reliably announced by screen readers, so the error `<p>` starts empty in `index.html`.
